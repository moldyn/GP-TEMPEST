...
from torch_geometric.nn import GATv2Conv, NNConv, SAGPooling, Set2Set
...
class InferenceGNN(nn.Module):
    """The graph neural network inference model."""
    def __init__(
        self,
        layers_encoder,
        layers_gaussian,
        ...
        node_features = 4
        edge_mlp_layers = [
            nn.Linear(edge_features, layers_encoder[0]),
            nn.BatchNorm1d(layers_encoder[0]),
            nn.LeakyReLU(),
            *[layer for i in range(len(layers_encoder) - 1) for layer in (
                nn.Linear(layers_encoder[i], layers_encoder[i + 1]),
                nn.BatchNorm1d(layers_encoder[i + 1]),
                nn.LeakyReLU(),
            )],
            nn.Linear(layers_encoder[-1], node_features * layers_encoder[-1]),
        ]
        self.edge_mlp = nn.Sequential(*edge_mlp_layers)
        self.conv1 = NNConv(
            in_channels=int(node_features),
            out_channels=int(layers_encoder[-1]),
            nn=self.edge_mlp,
            aggr='mean',
        )
        self.conv2 = GATv2Conv(
            in_channels=int(layers_encoder[-1]),
            out_channels=int(layers_encoder[-1]),
            edge_dim=edge_features,
            heads=4,
            concat=False,
        )
        self.batch_norm1 = nn.BatchNorm1d(layers_encoder[-1])
        self.batch_norm2 = nn.BatchNorm1d(layers_encoder[-1])
        self.dropout = nn.Dropout(0.2)
        self.set2set = Set2Set(int(layers_encoder[-1]), processing_steps=3)
        self.gaussian = GaussianLayer(layers_gaussian)
    ...
    def forward(self, data):
        ...
        x = self.conv2(data.x, data.edge_index, data.edge_attr)
        x = F.leaky_relu(x)
        x = self.dropout(x)
        x = self.batch_norm2(x)
        x = self.set2set(x, data.batch)
        mu, var, z = self.gaussian(x)
        return {
            'means': mu,
            'variances': var,
            'embedding': z,
...
class TEMPEST_GNN(nn.Module):
    def __init__(
        ...
        self.encoder = InferenceGNN(
            self.layers_encoder,
            layers_gaussian=[layers_hidden_encoder[-1], dim_latent],
        ).to(self.device).to(self.dtype)
        self.decoder = ConditionedGraphDecoder(
            dim_latent,
            layers_hidden_decoder[-1],
            dim_input,
            max_edges,
        ).to(self.device).to(self.dtype)
        # self.decoder.add_layer('sigmoid', nn.Sigmoid())  # TODO add?
        self.beta = beta
        self.N_data = N_data
    ...
    def gp_step(self, batch):
        # qzx = self.encoder(x)
        qzx = self.encoder(batch)
        qzx_mu = qzx['means']
        qzx_var = qzx['variances']
        self.compute_kernel_matrices(batch.t)
        gp_mean, gp_var = [], []
        loss_L3, loss_KL = [], []
